...
app.get('/status-info', (req, res) => {
  ...
  if (!code) {
      return res.status(400).json({ error: "Please provide a valid status code as a query parameter." });
  }

  const message = statusDescriptions[code];
  if (!message) {
      return res.status(404).json({ error: `Status code ${code} is not handled by this API.` });
  }
